...
- **Twilio**: SMS notifications, voicemail transcription, and voice/IVR services.
  ...
  - **Voicemail Email Alerts**: When voicemails are transcribed, an email notification is sent containing caller info, transcription text, AI summary, recording link, and dashboard link. Configured via `ALERT_EMAIL_TO` env var; gracefully skips if not set.
