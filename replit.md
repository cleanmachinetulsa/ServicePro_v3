...
**Phase 14 - Campaign-Aware AI Agent System** (Completed Nov 2025):
Intelligent AI agent integration making SMS and web chat agents fully aware of Welcome Back campaigns sent to customers. Core service (`server/services/campaignContextService.ts`) provides tenant-scoped campaign data lookup via `getCampaignContextForCustomer()`, returning recent campaign details (type, points bonus, channels, recency) and current loyalty points. Campaign tracking via `campaign_sends` table logs every SMS/Email delivery with campaign_key format ('welcome_back_v1_vip'/'welcome_back_v1_regular'), channel info, and timestamp. SMS prompt builder (`server/ai/smsAgentPromptBuilder.ts`) automatically injects CAMPAIGN CONTEXT section when customer received recent campaign (default 30-day window), including AI behavior rules to recognize and engage with campaign mentions. Web chat handler (`server/conversationHandler.ts`) provides campaign awareness for both restricted (anonymous) and authenticated modes via system prompt injection and message preprocessing. Global campaign handling rules prevent "I don't know" responses about campaigns, ensuring agents acknowledge and intelligently respond to campaign-related questions. Customer identification helpers (`getCustomerIdFromPhone`, `getCustomerIdFromEmail`) enable safe customer lookup for campaign context retrieval. All queries use strict tenant isolation via `tenantDb.withTenantFilter()` pattern, ensuring zero cross-tenant data leakage. Safe fallbacks throughout - AI agents never break if campaign lookup fails. Production-ready with comprehensive architect review validating tenant security, error handling, and multi-tenant isolation.
