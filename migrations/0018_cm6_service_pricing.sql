-- CM-6: Update all service names, prices, overviews, and descriptions.
-- Run as a single transaction — rolls back entirely on any error.
-- Apply with: psql "$DATABASE_URL" -f migrations/0018_cm6_service_pricing.sql
-- Or run:     npx tsx scripts/cm6RunMigration0018.ts

BEGIN;

-- ─── UPDATES ───────────────────────────────────────────────────────────────

-- ID 5: Maintenance Detail → Pit Stop
UPDATE services SET
  name = 'Pit Stop',
  price_range = '$179',
...
-- ID 4: Interior Detail → The Deep Clean
UPDATE services SET
  name = 'The Deep Clean',
  price_range = '$249',
...
-- ID 6: Paint Enhancement → Paint Revival
UPDATE services SET
  name = 'Paint Revival',
  price_range = '$249',
...
-- ID 7: Ceramic 1-Year → Force Field
UPDATE services SET
  name = 'Force Field',
  price_range = '$499',
...
-- ID 8: Ceramic 3-Year → Force Field Pro
UPDATE services SET
...
-- ID 9: Motorcycle Detail (repriced $150–175 → $199)
UPDATE services SET
...
-- ID 10: Premium Wash (repriced $75 → $99)
UPDATE services SET
  name = 'Premium Wash',
  price_range = '$99',
...
-- ID 11: Shampoo Seats/Carpets → Carpet & Upholstery Shampoo
UPDATE services SET
...
-- ID 12: Leather or Fabric Protector (repriced $50 → $79)
UPDATE services SET
...
-- ID 13: Light Polish → retire (check booking count first!)
-- If 0 bookings from Step 1: delete. If bookings exist: rename to retired.
-- This script uses the safe retire-via-rename approach.
-- To delete instead: change to DELETE FROM services WHERE id = 13 AND tenant_id = 'root';
UPDATE services SET
  name = '[Retired] Light Polish'
WHERE id = 13 AND tenant_id = 'root';

-- ID 14: Headlight Restoration (repriced $25 → $90)
UPDATE services SET
...
-- ID 15: 12-Month Ceramic duplicate → retire (check booking count first!)
-- Same pattern as ID 13 — safe rename approach.
UPDATE services SET
  name = '[Retired] 12-Month Ceramic'
WHERE id = 15 AND tenant_id = 'root';

-- ID 16: Plastic Trim Restoration (repriced $40 → $65)
UPDATE services SET
...
-- ID 17: Engine Bay Cleaning (repriced $35 → $75)
UPDATE services SET
...
-- ID 18: Glass/Windshield Protector (repriced $30 → $49)
UPDATE services SET
...
-- ID 19: Ozone Odor Removal (repriced $50 → $75)
UPDATE services SET
...
-- NEW: Showstopper ($549) — full detail + single-stage paint enhancement
...
-- NEW: Paint Restoration ($399) — two-stage, exterior only
INSERT INTO services (
  tenant_id, name, price_range, overview, detailed_description,
  duration, duration_hours, min_duration_hours, max_duration_hours
) VALUES (
