...
export async function generateAIResponse(
  ...
  try {
    ...
    // Get campaign context for campaign-aware responses
    const campaignContext = phoneNumber ? await getCampaignContextByPhone(phoneNumber) : { hasRecentCampaign: false };
    ...
    const baseSystemPrompt = `You are a virtual assistant for Clean Machine Auto Detail, a mobile auto detailing service in Tulsa, OK. 
        ...
        9. CRITICAL ESCALATION RULE: If you cannot complete an action or encounter a technical issue, ALWAYS include the phone number in your response. Say: "I apologize for the issue. Please call us at 918-856-5711 and we'll get this sorted out right away." NEVER say "call our contact number" or "give us a call" without including the actual phone number 918-856-5711.
        ...
        - If the customer references:
          ...
          4) Offer a concrete next step: checking their points, booking a service, or accessing their account.
    ...
    let personalizedSystemPrompt = buildPersonalizedSystemPrompt(baseSystemPrompt, customerContext);
    
    // Inject campaign context if customer has recent campaigns (Phase 2)
...
- Instead:
  ...
  3) Offer a clear next step:
     - help them book using those points, or
     - help them access their account / booking link.
