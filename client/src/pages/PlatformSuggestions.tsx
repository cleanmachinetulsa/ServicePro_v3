...
import { MessageSquare, CheckCircle2, RotateCcw, Loader2, Trash2, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function PlatformSuggestionsPage() {
  ...
  const { data, isLoading, error } = useQuery<{ success: boolean; suggestions: Suggestion[] }>({
    queryKey: ['/api/suggestions/platform'],
  });
  ...
  const handleMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/suggestions/platform'] });
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 px-4">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold">ServicePro Feedback Inbox</h1>
          <p className="text-sm text-muted-foreground">
            Suggestions and feedback from platform users about ServicePro itself
          </p>
        </div>
        {openCount > 0 && (
